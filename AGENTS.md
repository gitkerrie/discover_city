...
修改城市内容时优先编辑 `js/data.js`。每座城市必须拥有唯一的英文 `slug`、3 个风味标签、3-5 道招牌美食和本地 WebP 主图。图片变更需同步更新 `assets/cities/SOURCES.md`。

## 测试指南

提交前运行 `npm run check`，再通过浏览器验证地图点位、城市与菜名搜索、详情抽屉、分享深链接、收藏持久化及旧收藏迁移。至少检查一个桌面视口和一个手机视口，并确认图片失败状态和 `Esc` 关闭操作正常。
